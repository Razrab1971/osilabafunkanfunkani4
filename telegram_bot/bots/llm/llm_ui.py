...
from connect_ai.model_ai_core import ModelAI


# LLM_MODE_KEY = "llm_chat_mode"
INTERCEPTOR_MODE = "interceptor_mode"

# Для DuckDuckGo - не имеет историю, поэтому у него без настроек сразу прокидываем в чат


async def activate_duckduckgo_llm(update: Update, context: ContextTypes.DEFAULT_TYPE, model: ModelAI):
    await context.bot.send_message(
        chat_id=update.effective_chat.id, 
        text="AI на основе G4f готов принимать ваши запросы"
    )

    context.user_data[INTERCEPTOR_MODE] = model
